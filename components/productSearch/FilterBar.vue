<template>
  <div
    class="flex justify-between items-center bg-white rounded-lg py-2 px-4 shadow-[5px_12px_35px_rgba(44,44,44,0.12)]"
  >
    <div class="flex flex-1 max-w-full">
      <div class="flex gap-x-4 items-center">
        ...
        <SelectComponent
          @onGetData="onGetCountries"
          :options="countries"
          :showSearch="true"
          placeholder="Country"
          classStyles="appearance-none h-[46px] hidden uppercase text-matta-black bg-transparent py-2 px-3 min-w-[150px] cursor-pointer md:py-3 md:px-6 border !rounded-full border-[#ddd] md:leading-5 text-[10px] !sm:text-[13px] shadow-sm focus:outline-gray-200"
        />
...
const filteredproducers = computed(() => {
  return producers.value.map((i) => {
    return { id: i.title, name: i.title.toLowerCase(), value: i.title };
  });
});
...
function onGetPrice(data) {
  queryParams.Pricefilter = data.value;
}
