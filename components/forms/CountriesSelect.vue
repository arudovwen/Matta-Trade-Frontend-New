<template>
  <Listbox v-model="selectedOption">
    <div class="relative w-full">
      ...
      >
        <ListboxOptions
          class="absolute mt-1 w-full min-w-[320px] border border-gray-200 z-40 rounded-lg bg-white py-3 text-xs shadow-lg ring-1 ring-black ring-opacity-5 outline-0 sm:text-[13px]"
        >
          <div class="relative flex items-center mb-3 px-4">
            <input
              v-model="query"
              placeholder="Search"
...
onMounted(() => {
  ...
  selectedOption.value = countries.find((i) => i.name == props.modelValue);
});
const query = ref("");
const filteredCountries = computed(() => {
  if (!query.value.length) return countries;
